...
import { TRAFFIC_VULNERABILITY_BY_NAME } from "./traffic-vulnerability-data";

type AreaCode = string;
type PilotAreaName = "광덕면" | "풍세면" | "청룡동" | "신방동";
type ComparisonMetric = "all" | "elderly-count" | "elderly-rate" | "transport" | "age-band" | "living-alone" | "disabled";
...
const AGE_COMPOSITION_COLORS = ["#2563eb", "#ff0000", "#fffb00"];
...
export default function Home() {
  ...
  const trafficVulnerability = TRAFFIC_VULNERABILITY_BY_NAME[pilotName(selectedArea)];
  ...
  const comparisonBaseCandidates = comparisonScope === "top10" ? rankedAreas.slice(0, 10) : [...pilotAreas];
  const comparisonCandidates = comparisonBaseCandidates.includes(selectedArea)
    ? comparisonBaseCandidates
    : [selectedArea, ...comparisonBaseCandidates];
  ...
  const comparisonAreas = sortedComparisonAreas;
  const comparisonOrder = new Map(comparisonAreas.map((area, index) => [area, index + 1]));
  const comparisonAreaSplit = Math.ceil(comparisonAreas.length / 2);
  const comparisonAreaColumns = [
    comparisonAreas.slice(0, comparisonAreaSplit),
    comparisonAreas.slice(comparisonAreaSplit),
  ];
  const maxElderly = Math.max(...pilotAreas.map((area) => POPULATION_DATA[area].elderly));
  const maxTransportScore = 100;
      ...
      <aside ref={detailCard} className={`area-card is-${profile.tone}`}>
        ...
        <section className="vulnerability-panel" aria-labelledby="vulnerability-title">
          ...
          <details className="calculation-details">
            ...
            <div className="calculation-lead">
              <strong>{selectedModel.label} 모형으로 계산</strong>
              <span>{selectedModel.description} · 점수 범위 0~100점 · 천안시 31개 행정구역 내부 상대 비교</span>
            </div>
            <div className="calculation-score-grid">
              {[
                ["인구취약", vulnerability.populationScore, selectedModel.weights.population],
                ["의료취약", vulnerability.medicalScore, selectedModel.weights.medical],
                ["교통취약", vulnerability.transportScore, selectedModel.weights.transport],
              ].map(([label, score, weight]) => (
                <div key={label as string}>
                  <span>{label as string}</span>
                  <b>{(score as number).toFixed(1)}점</b>
                  <small>{formatWeight(weight as number)} 반영 · 기여 {(score as number * weight as number).toFixed(2)}점</small>
                </div>
              ))}
            </div>
            <p className="calculation-formula">
              종합점수 = 인구취약 {(vulnerability.populationScore * selectedModel.weights.population).toFixed(2)} + 의료취약 {(vulnerability.medicalScore * selectedModel.weights.medical).toFixed(2)} + 교통취약 {(vulnerability.transportScore * selectedModel.weights.transport).toFixed(2)} = <b>{vulnerability.overallScore.toFixed(1)}점</b>
            </p>
            <div className="calculation-section">
              <strong>교통취약도에 사용한 파라미터</strong>
              {trafficVulnerability ? <>
                <p>① 일일 운행 공급량: 노선-정류장 연결을 중복 제거한 뒤 노선별 <b>1일 운행횟수(회·편도 기준)</b>를 합산했습니다. {pilotName(selectedArea)}은 <b>{trafficVulnerability.dailySupply.toLocaleString()}회/일</b>입니다.</p>
                <p>② 정류장 밀도: 고유 정류장 {trafficVulnerability.uniqueStops.toLocaleString()}개 ÷ GeoJSON 면적 {trafficVulnerability.areaKm2.toFixed(2)}㎢ = <b>{trafficVulnerability.stopDensity.toFixed(2)}개/㎢</b>입니다.</p>
                <p>③ 31개 지역의 상대 비교: 운행 공급량 부족도 <b>{trafficVulnerability.supplyShortage.toFixed(2)}점</b>, 정류장 밀도 부족도 <b>{trafficVulnerability.densityShortage.toFixed(2)}점</b>으로 계산했습니다.</p>
                <p>교통취약점수 = 운행 공급량 부족도 50% + 정류장 밀도 부족도 50% = <b>{trafficVulnerability.score.toFixed(1)}점</b> · 교통취약순위 {trafficVulnerability.rank}위</p>
              </> : <p>선택 지역의 traffic_calc 상세 결과가 없습니다.</p>}
            </div>
            <div className="calculation-section">
              <strong>이번 결과의 해석</strong>
              <p>주요 취약영역은 <b>{vulnerability.coreArea || "인구·의료·교통"}</b>으로 분류되었습니다. {selectedModel.label}의 가중치를 적용한 결과 종합 <b>{vulnerability.overallScore.toFixed(1)}점</b>으로 {vulnerability.rank}위, <b>{vulnerability.grade}등급·{vulnerability.level}</b>으로 분류되었습니다.</p>
              <p>이 점수는 개인의 의료 필요도를 뜻하는 진단값이 아니라, 순회진료 지원 우선순위를 정하기 위한 행정구역 간 상대 비교값입니다.</p>
            </div>
            <p>참고: 고령인구 밀도 {getElderlyDensity(selectedArea).toFixed(1)}명/㎢ · 고령인구 1,000명당 정류장 {transportMetrics.supply.toFixed(1)}개는 교통취약도 산식의 원점수에는 직접 합산하지 않는 보조지표입니다.</p>
          </details>
        </section>
          ...
          >
            ...
            <section className="metric-focus-card" aria-labelledby="comparison-metric-title">
              ...
              {comparisonMetric === "all" ? (
                <div className="metric-focus-all-note">
                  전체 항목을 선택하면 아래에 인구·연령·독거노인·장애인·취약점수 비교를 모두 표시합니다.
                  특정 항목을 선택하면 해당 항목만 집중해서 볼 수 있습니다.
                </div>
              ) : (
                <>
              {comparisonMetric === "elderly-count" && (
                <div className="focus-bar-chart">
                  ...
                  {comparisonAreaColumns.map((column, columnIndex) => (
                    <div className="focus-bar-column" key={columnIndex}>
                      {column.map((area) => {
                        const value = POPULATION_DATA[area].elderly;
                        return (
                          <div className="focus-bar-row" key={area}>
                            <strong>{comparisonOrder.get(area)}. {pilotName(area)}</strong>
              ...
              {comparisonMetric === "transport" && (
                <>
                <p className="metric-focus-data-note">
                  교통취약점수는 `일일 운행 공급량 부족도`와 `정류장 밀도 부족도`를 각각 50% 반영한 값입니다. 두 지표는 천안시 31개 행정구역 안에서 역 min-max 정규화했으며, 점수가 높을수록 교통 여건이 취약하다는 뜻입니다.
                </p>
                <div className="focus-bar-chart">
                  <div className="focus-chart-axis"><span>0점</span><span>100점</span></div>
                  {comparisonAreaColumns.map((column, columnIndex) => (
                    <div className="focus-bar-column" key={columnIndex}>
                      {column.map((area) => {
                        const value = getAreaVulnerability(area).transportScore;
                        return (
                          <div className="focus-bar-row" key={area}>
                            <strong>{comparisonOrder.get(area)}. {pilotName(area)}</strong>
                            <div className="focus-bar-track is-transport"><i style={{ width: `${(value / maxTransportScore) * 100}%` }} /></div>
                            <b>{value.toFixed(1)}점</b>
                          </div>
                        );
                      })}
                    </div>
                      ...
                      {comparisonAreas.map((area) => {
                        ...
                        return (
                          <div className="focus-band-row" key={area}>
                            <span>{comparisonOrder.get(area)}. {pilotName(area)}</span>
              ...
              {comparisonMetric === "living-alone" && (
                <>
                <p className="metric-focus-data-note">
                  2019년 원자료가 통합 불당동으로 제공되어 불당1·2동은 2026년 65세 이상 인구 비율로 추정 배분했습니다.
                </p>
                <div className="focus-stack-chart">
                  {comparisonAreaColumns.map((column, columnIndex) => (
                    <div className="focus-stack-column" key={columnIndex}>
                      {column.map((area) => {
              ...
              <p className="metric-focus-note">
                선택 항목의 막대 길이는 지역 간 비교를 위한 상대 크기입니다. 현재 {comparisonAreas.length}개 지역 표시 · {comparisonScope === "top10" ? "취약 상위 10개 + 선택 지역" : "전체 31개 지역"} · {COMPARISON_SORT_LABELS[comparisonSort]} · 선택 지역은 강조 표시됩니다.
              </p>
              <div className="comparison-source-note">
                <strong>사용 자료·산정 기준</strong>
                <p><b>인구·연령:</b> KOSIS 「101_DT_1B04005N_M_2026.csv」(2026년 6월, 천안시 31개 행정구역 5세별 주민등록인구)</p>
                <p><b>독거노인:</b> 「독거노인_현황_성별__20260723173327.xlsx」(2019년), 독거노인·기초생활보장·저소득노인 구성</p>
                <p><b>의료·약국·장애인:</b> 「의료기관_20260723172839.xlsx」·「의약품_등_판매업소_20260723173542.xlsx」(2024년), 「장애인_등록현황_20260723173318.xlsx」(2023년)</p>
                <p><b>취약점수·지도:</b> 「지도제작용_등급데이터.csv」·「최종_취약지역_설명_정책추천.csv」·「동일가중치_CRITIC_가중치_비교분석.xlsx」·천안시 행정구역 GeoJSON</p>
                <p><b>교통:</b> 국토교통부 전국 버스정류장 위치정보(2025년 10월 31일 기준), 행정구역 경계 안의 정류장 위치를 지역별로 집계</p>
                <p className="comparison-source-estimate"><b>불당1·2동 독거노인 추정:</b> 2019년 원자료의 통합 불당동 값(독거노인 323명·기초생활 66명·저소득 6명·일반 251명)을 2026년 65세 이상 인구 비율로 배분했습니다. 불당1동 2,234명(59.4%), 불당2동 1,528명(40.6%)을 적용해 각각 192·131명, 39·27명, 4·2명, 149·102명으로 계산했으며 실제 2019년 동별 실측값이 아닌 추정값입니다.</p>
              </div>
                </>
              )}
            </section>
              ...
              <div className="age-composition-layout">
                ...
                <div className="age-stack-compare">
                  {comparisonAreas.map((area) => {
                    ...
                    if (!values) return <div className="age-stack-row" key={area}><strong>{comparisonOrder.get(area)}. {pilotName(area)}</strong><b>자료 없음</b></div>;
            ...
            <div className="comparison-grid">
                        ...
                        {column.map((area) => {
                          ...
                          return (
                            <div className="vulnerability-graph-row" key={area}>
                              ...
                              <strong>{comparisonOrder.get(area)}. {pilotName(area)}</strong>
                              <span>{summary.grade}등급 · 취약 {summary.rank}위</span>
                              </div>
                      ...
                      {comparisonAreaColumns.map((column, columnIndex) => (
                        <div className="band-column" key={columnIndex}>
                          {column.map((area) => (
                            ...
                            <small>{comparisonOrder.get(area)}. {pilotName(area)}</small>
                              {(() => {
                                const value = POPULATION_DATA[area].ageBands?.[index] ?? null;
              ...
              <article className="chart-card is-wide">
                <div className="chart-title">
                  ...
                  <span>2019년 · 불당1·2동은 고령인구 비율 추정 배분</span>
                </div>
                {comparisonAreas.map((area) => {
                  const item = POPULATION_DATA[area];
                  const available = item.livingAlone !== null && item.basicLiving !== null && item.lowIncome !== null;
                ...
                <p className="chart-data-note">
                  원자료의 불당동 통합값을 2026년 65세 이상 인구 비율로 추정 배분한 값입니다.
                </p>
              </article>
            ...
            <footer className="source-note">
              ...
              독거노인 하위구성 중 불당1·2동 값은 2019년 통합 불당동 자료의 추정 배분값입니다.
